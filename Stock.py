    ...
    def __init__(self, stock_number):
        ...
        if len(result) == 0:
            ...
            create_cmd = "create table `{0}` ".format(self.stock_number) + \
                         "(id int auto_increment, time timestamp, open_val float(8,3), close_val float(8,3), high_val float(8,3), low_val float(8,3), total_val float(25), total_amount float(25), week_day tinyint, EMA12 float(8,3) default 0, EMA26 float(8,3) default 0, DIF float(8,3) default 0, DEA float(8,3) default 0, primary key (id));"
            print create_cmd
            try:
